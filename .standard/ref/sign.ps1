# Sign <EXE> (Authenticode). Run AFTER build.ps1 - rebuild strips the signature.
...
param(
    [string]$Exe = (Join-Path (Split-Path -Parent $MyInvocation.MyCommand.Path) "<EXE>"),
    [string]$Subject  = "<CERT_SUBJECT>",
    [string]$Friendly = "<APP> Code Signing (<CERT_ORG>)",
...
#      - The REAL cert's friendly name is "PuntoFree Code Signing (<CERT_ORG>)" (a pre-rename
...
$Pinned = "<THUMBPRINT>"   # must match src\Updater.cs
...
$cer = Join-Path (Split-Path -Parent $Exe) "<CER>"
